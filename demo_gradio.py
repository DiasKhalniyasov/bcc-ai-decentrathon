...
# Add OllamaQwen3Predictor import
from ollama_qwen3_predictor import OllamaQwen3Predictor
...
# Create OllamaQwen3Predictor instance
contract_predictor = OllamaQwen3Predictor(model_name="gemma3:27b")
...
def get_initial_session_state():
    return {
        'processing_results': {
            ...
            'result_paths': None,
            'pdf_results': None,
            'contract_data': None  # Added for contract extraction results
        },
        'pdf_cache': {
            "images": [],
            "current_page": 0,
...
def load_file_for_preview(file_path, session_state):
    ...
    try:
        if file_ext == '.pdf':
            ...
            session_state['pdf_cache']["file_type"] = "pdf"
        elif file_ext in ['.jpg', '.jpeg', '.png']:
            image = Image.open(file_path)
            pages = [image]
            session_state['pdf_cache']["file_type"] = "image"
        else:
    ...
    session_state['pdf_cache']["images"] = pages
    session_state['pdf_cache']["current_page"] = 0
    session_state['pdf_cache']["total_pages"] = len(pages)
    session_state['pdf_cache']["is_parsed"] = False
    session_state['pdf_cache']["results"] = []
    
    # Reset contract data when loading new file
    session_state['processing_results']['contract_data'] = None
...
def turn_page(direction, session_state):
    ...
    if not pdf_cache["images"]:
        return None, "<div id='page_info_box'>0 / 0</div>", "", "No contract data available", session_state
    ...
    current_json = ""
    # Get contract data from processing_results (document-level data)
    current_contract_data = get_contract_data_for_display(session_state)
    ...
    return current_image, page_info, current_json, current_contract_data, session_state

def get_contract_data_for_display(session_state):
    """Get contract data for display, preserving it across page turns"""
    processing_results = session_state['processing_results']
    
    if processing_results.get('contract_data'):
        try:
            return json.dumps(processing_results['contract_data'], ensure_ascii=False, indent=2)
        except Exception:
            return str(processing_results['contract_data'])
    else:
        return "No contract data extracted"
...
def extract_contract_data(cells_data):
    """Extract contract data using OllamaQwen3Predictor"""
    if not cells_data:
        return None
    
    try:
        # Check if model is available
        if not contract_predictor.check_model_available():
            print("Ollama model not available, trying to pull...")
            if not contract_predictor.pull_model():
                print("Failed to pull model, skipping contract extraction")
                return None
        
        # Extract contract data using the predictor
        contract_data = contract_predictor.predict(json_data=cells_data)
...
def parse_image_with_high_level_api(parser, image, prompt_mode, fitz_preprocess=False):
    ...
    try:
        ...
        # Extract contract data if cells_data is available
        contract_data = None
        if cells_data:
            print("Extracting contract data from layout results...")
            contract_data = extract_contract_data(cells_data)
...
def parse_pdf_with_high_level_api(parser, pdf_path, prompt_mode):
    ...
    try:
        ...
        # Extract contract data from combined cells data
        contract_data = None
        if all_cells_data:
            print("Extracting contract data from PDF layout results...")
            contract_data = extract_contract_data(all_cells_data)
    ...
    try:
        if file_ext == '.pdf':
            ...
            processing_results.update({
                ...
                'contract_data': pdf_result['contract_data'],
                    ...
                    for root, _, files in os.walk(pdf_result['temp_dir']):
                        for file in files:
                            if not file.endswith('.zip'):
                                zipf.write(os.path.join(root, file), os.path.relpath(os.path.join(root, file), pdf_result['temp_dir']))

            # Format contract data for display
            contract_json = ""
            if pdf_result['contract_data']:
                contract_json = json.dumps(pdf_result['contract_data'], ensure_ascii=False, indent=2)
            else:
                contract_json = "No contract data extracted"
            ...
            return (
                ...
                gr.update(value=download_zip_path, visible=bool(download_zip_path)), page_info, current_page_json, contract_json, session_state
            )
            ...
            if parse_result['filtered']:
                 ...
                 processing_results.update({
                     ...
                     'contract_data': parse_result.get('contract_data'),
                 ...
                 # Format contract data for display
                 contract_json = ""
                 if parse_result.get('contract_data'):
                     contract_json = json.dumps(parse_result['contract_data'], ensure_ascii=False, indent=2)
                 else:
                     contract_json = "No contract data extracted (JSON parsing failed)"
                 
                 return original_image, info_text, parse_result['md_content'], parse_result['md_content'], gr.update(visible=False), None, "", contract_json, session_state
            ...
            processing_results.update({
                ...
                'contract_data': parse_result['contract_data'],
                'temp_dir': parse_result['temp_dir'], 'session_id': parse_result['session_id'],
                'result_paths': parse_result['result_paths']
            })
            ...
            return (
                ...
                None, current_json, contract_json, session_state
            )
    except Exception as e:
        import traceback
        traceback.print_exc()
        return None, f"Error during processing: {e}", "", "", gr.update(value=None), None, "", "Error during processing", session_state
...
def clear_all_data(session_state):
    ...
    return (
        ...
        "🕐 Waiting for parsing result...",     # Clear contract data
        new_session_state
    )
    ...
    with gr.Blocks(theme="ocean", css=css, title='dots.ocr') as demo:
        ...
        gr.HTML("""
            <div style="display: flex; align-items: center; justify-content: center; margin-bottom: 20px;">
                <h1 style="margin: 0; font-size: 2em;">BCC AI OCR</h1>
            </div>
                    ...
                    with gr.Column(scale=3):
                        ...
                        with gr.Tabs(elem_id="markdown_tabs"):
                            ...
                            with gr.TabItem("Data Extraction"):
                                data_extraction_json = gr.Textbox(
                                    value="🕐 Waiting for parsing result...",
        ...
        prev_btn.click(
            ...
            outputs=[result_image, page_info, current_page_json, data_extraction_json, session_state]
        )
        ...
        process_btn.click(
            ...
            outputs=[
                ...
                download_btn, page_info, current_page_json, data_extraction_json, session_state
            ]
        )
